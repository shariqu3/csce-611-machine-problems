...
/* The low-level functions (defined in file 'irq_low.s') that handle the
   ...
   Yes, there are more efficient ways to handle exceptions, but they require
   more code replication.
*/
...
InterruptHandler
    *InterruptHandler::handler_table[InterruptHandler::IRQ_TABLE_SIZE];
...
void InterruptHandler::send_eoi(unsigned int _irq_no) {
  if (generated_by_slave_PIC(_irq_no)) {
...
void InterruptHandler::acknowledge_interrupt(REGS *_r) {
  unsigned int irq_no = _r->int_no - IRQ_BASE;
  assert(irq_no < IRQ_TABLE_SIZE);

  send_eoi(irq_no);
  _r->err_code = EOI_ALREADY_SENT_MARK;
}
...
void InterruptHandler::dispatch_interrupt(REGS *_r) {
  ...
  if (_r->err_code != EOI_ALREADY_SENT_MARK) {
    send_eoi(int_no);
  } else {
    _r->err_code = 0;
  }
