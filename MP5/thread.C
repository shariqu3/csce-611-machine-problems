...
inline void Thread::push(unsigned long _val) {
  /* This function is originally borrowed from David H. Hovemeyer
   * <daveho@cs.umd.edu> */
  esp -= 4;
...
static void thread_shutdown() {
  /* This function should be called when the thread returns from the thread
     function. It terminates the thread by releasing memory and any other
...
Thread::Thread(Thread_Function _tf, char *_stack, unsigned int _stack_size) {
  /* Construct a new thread and initialize its stack. The thread is then ready
     to run. (The dispatcher is implemented in file "thread_scheduler".)
  */
...
void Thread::dispatch_to(Thread *_thread) {
  /* Context-switch to the given thread. Calls the low-level context switch code
     in thread_low.asm.
     NOTE: This call does not return until after the current thread is switched
     back in. NOTE: We don't consider the system start thread as an actual
     thread. Therefore, we will not return from this function ever when the
     system start code (in kernel.C) starts up the first thread.
  */
  ...
  /* The call does not return until after the thread is context-switched back
   * in. */
}
