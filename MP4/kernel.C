...
void GeneratePageTableMemoryReferences(unsigned long start_address,
                                       int n_references);
...
// replace the operator "delete[]"
void operator delete[](void *p) { current_pool->release((unsigned long)p); }
...
and overload the method handle_exception. */
  ...
  PageTable::init_paging(&kernel_mem_pool, &process_mem_pool, 4 MB);
...
#define _TEST_PAGE_TABLE_
  ...
  /* ---- We define the code pool to be a 256MB segment starting at virtual
   * address 512MB -- */
